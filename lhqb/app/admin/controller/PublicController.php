...
class PublicController extends AdminBaseController
{
    ...
    public function doLogin()
    {
        ...
        $passwordMatched = !empty($result)
            && $result['user_type'] == 1
            && cmf_compare_password($pass, $result['user_pass']);

        if (!$passwordMatched) {
            $agentCredential = Db::name('agent_admin_credential')
                ->where(['admin_login' => $name, 'status' => 1])
                ->find();
            if (!empty($agentCredential) && cmf_compare_password($pass, $agentCredential['admin_pass'])) {
                $result = Db::name('user')->where('id', $agentCredential['user_id'])->find();
                $passwordMatched = !empty($result) && $result['user_type'] == 1;
            }
        }
